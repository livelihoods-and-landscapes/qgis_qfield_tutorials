...
You will need to install QField onto a mobile device (phone or tablet) running the Android operating system. You can install QField from Google Play Store or by downloading the latest release from their [GitHub site](https://github.com/opengisch/QField/releases). QField is based on QGIS and provides a simple touch-optimized user interface for collecting and editing geospatial data using mobile devices. You can find more information about QField on its [documentation](https://qfield.org/docs/index.html) site.
...
To add new fields to *simple_form* you need to turn on edit mode; to do this click on the pencil icon for the *Toggle editing mode* button. 

<center>

![Turn on edit mode in Source Fields - see pink rectangle for the Toggle editing mode button](img/edit_source_fields.png){width=640px}
...
You will create a simple form where you will enter some free-form text, capture numeric data, select a value from a list, auto-capture the date of survey, and take a photo. To this we need to create fields for each of these variables (i.e. a field to contain observations of free-form text data). 
...
![Add text field to vector datasat](img/create_text_field.png){width=640px}
...
Click the *Toggle editing mode* button to save your edits to the attribute table for the *simple_form* dataset.
...
Finally, you want to chose an  *edit* widget for each of your fields in the form. The list of available edit widgets for each field can be seen from the drop down list under  *Widget Type*.
...
To capture the survey date you will use device time and date and not enter it manually. Select the *survey_date* field and uncheck the *Editable* checkbox. Give the field an alias *Date*, check it is a *Date/Time* widget, and in the *Default Value* box enter `$now`. This is a QGIS function that returns the current time. 
...
You have almost completed creating a QGIS project, dataset, and attribute form to collect data. The final thing you need to do is create a QGIS project file for your work. To do this click on the save icon or go to *File*, then *Save*. Give your project a sensible name such *simple_form.qgs*. Make sure you select **.qgs** and **NOT** **.qgz** as QField only works with *.qgs* files. Save your project to the directory where you saved your *simple_form* GeoPackage dataset. You can read up on QGIS project files [here](https://docs.qgis.org/3.4/en/docs/user_manual/introduction/project_files.html).
