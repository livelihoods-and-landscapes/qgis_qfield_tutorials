...
Follow the steps in *Field Creation* in *Lesson 1* to create the following fields: 
...
To set up your form to compute the area select the *field_area* field in *Available Widgets* then give the field an alias *area of field?* and uncheck the *Editable* checkbox. Next, go to *Defaults* and enter `$area`. When you digitze a polygon in QField the value returned from executing the `$area` function will be assigned to *field_area*.
...
![Add foriegn key field](img/create_foreign_key.png){width=640px}

</center>

#### Add Relation

Having created a foreign key and other fields in your *many_records* table you need to add the relation to the *complex_form* table. To do this go the *Project* menu then *Properties*. In *Project Properties* select the *Relations* tab on the left hand toolbar. 
...
Finally, you need to set up the form to enter data into *many_records*. Go the the *Properties* then *Attribute Form* of *many_records*. First, setup the foreign key *fk_field* as a *Relation Reference* widget. Make sure you uncheck the *Editable* checkbox. 
...
You have now set up a form with constraints and a one-to-many relation. Save the project as a *.qgs* file called *complex_form.qgs* then configure the project using QField Sync. Then package the project into the directory *export_complex_form*. Make sure you set you tables to offline editing during project configuration. Copy the *export_complex_form* folder to your mobile device. 
