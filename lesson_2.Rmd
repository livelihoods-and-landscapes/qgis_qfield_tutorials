...
Next, you need to zoom in on Fiji. To speed up the zoom process and rendering of the Open Street Maps layer change the project projection system to EPSG: 3857. You can do this by selecting the current CRS for the project in the bottom right corner of the QGIS interface and selecting EPSG: 3857.
...
Once you have changed the CRS you can use the zoom icon in the toolbar to zoom in on Fiji. If you zoom in on Suva your *Map Canvas* should look like the image below.
...
You will need to configure your project before packaging it for QField. To do this go to the *Plugins* menu then select *QFieldSync* and *Project Configuration*. 
...
Connect your Android device to your desktop. Copy the *export_form* directory to your Android device's internal storage (**NOT** the SD card). 

## QField

Open QField by selecting the QField icon ![](qfield.png){width=3.5em} on your Android device. 
...
You can manually digitise points by moving the crosshairs cursor on the *Map Canvas* and using the basemap images as a guide. However, you can also track your GPS position and add vertices for your current location. You can do this by selecting the GPS icon in the bottom right hand corner highlighted in the image below. 
