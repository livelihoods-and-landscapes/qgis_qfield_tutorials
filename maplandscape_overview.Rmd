...
<a href="https://fiji-ag-dot-aciarlal1.ts.r.appspot.com/" target="_blank">Data and training home</a>.

## Quick Tour

This tutorial is a demo map.landscape using fake data generated through the Fiji Government's Ministry of Agriculture *farmer profile form*. This tutorial will introduce a workflow to explore data related to dalo production on subsistence, semi-subsistence, and commercial farms.  
...
Follow these operations to load the  move to the *farmer-profile-form.gpkg* data into the dashboard:
...
Click the **Map controls** check box to bring up the options to style your web map. To quickly expore the data we'll make a map of the fields surveyed using the *farmer profile form* and visualise the area of each field using a colour scale. 
...
3. Choose a **Fill colour palette** such as *red-yellow-green* - with this colour palette fields with a smaller area will be visualised in red shades and fields with larger areas will be visualised in green shades. 
...
Next, we will calculate the area of each field under dalo cultivation. The <em>dalo</em> has an *area.x* column which records the area of the field (computed by the GIS on QField) and an *area.y* column which records the percentage of a field under dalo cultivation. We can combine values in these two columns to calculate the area of dalo in square metres in a new column. 
...
In the popup window, enter *the following into <em>dalo_area</e> into the **New column name** and `area.x * (area.y / 100)` into **Function to add new column**. 
